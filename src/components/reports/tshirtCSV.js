...
class TshirtCSV extends Component {
  ...
  getUserList = () => {
    return fetch('http://localhost:8080/api/tshirt')
      .then(res => res.json());
  }
  ...
  downloadReport = async () => {
    ...
    console.log(data);
    this.setState(
        {data:[ ["Description & Size","Supima Cotton" ,"Sweat-wicking"],
  ...
  render() {
    ...
    return (
      <div>
        <Button size="large" variant="contained" onClick={this.downloadReport}>Click to downloadReport</Button>
        <CSVLink
          //headers={headers}
